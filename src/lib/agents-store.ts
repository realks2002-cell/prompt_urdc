...
export async function readAgents(): Promise<Prompt[]> {
  if (useBlob()) {
    console.log("[AgentStore] Vercel Blob에서 읽기 시도");
    const data = await readBlobJson<Prompt[]>(BLOB_PATH);
    return Array.isArray(data) ? data : [];
  }
  try {
    console.log(`[AgentStore] 로컬 파일에서 읽기: ${FILE_PATH}`);
    const raw = await readFile(FILE_PATH, "utf-8");
    const data = JSON.parse(raw) as Prompt[];
    return Array.isArray(data) ? data : [];
  } catch (error) {
    console.error("[AgentStore] 로컬 파일 읽기 실패:", error);
    return [];
  }
}
...
export async function writeAgents(items: Prompt[]): Promise<void> {
  if (useBlob()) {
    console.log(`[AgentStore] Vercel Blob에 저장 시도 (${items.length}개 항목)`);
    await writeBlobJson(BLOB_PATH, items);
    return;
  }
  try {
    console.log(`[AgentStore] 로컬 파일에 저장: ${FILE_PATH} (${items.length}개 항목)`);
    await writeFile(FILE_PATH, JSON.stringify(items, null, 2), "utf-8");
    console.log("[AgentStore] 로컬 파일 저장 성공");
  } catch (error) {
    console.error("[AgentStore] 로컬 파일 저장 실패:", error);
